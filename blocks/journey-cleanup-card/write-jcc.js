...
const out = path.join(__dirname, 'journey-cleanup-card.js');

const code = `/* Journey Cleanup Dashboard - AJO stale journeys (30+ days) */
...
function showModal(onOk, onCancel) {
  ...
  box.innerHTML = [
    ...
    \`    <textarea id="jcc-m-token" name="token" rows="4" placeholder="Paste your Bearer access token here...">\${esc(saved.token || '')}</textarea>\`,
    ...
    \`      <input id="jcc-m-tenant" name="tenantId" type="text" value="\${esc(saved.tenantId || '')}" placeholder="e.g. adobe-corpnew" />\`,
  ...
  // Cancel button — only shown when reconfiguring (onCancel provided)
...
function showDashboard(root, cfg) {
  ...
  function mkDetail(j) {
    ...
    const detailRows = [
      ...
      ['Bucket', \`<span class="jcc-bucket-badge jcc-bucket-\${stCls}">\${staleBucket(days)}</span>\`, null, false],
      ['Journey URL', \`<a class="jcc-link" href="\${esc(journeyUrl)}" target="_blank" rel="noopener noreferrer">\${esc(journeyUrl)}</a>\`, journeyUrl, true],
    ];
    let grid = '<div class="jcc-dgrid">';
    detailRows.forEach(([lbl, val, copyv, full]) => {
      const fc = full ? ' jcc-df' : '';
      const cb = copyv ? \`<button class="jcc-copy" data-v="\${esc(copyv)}">&#x1F4CB;</button>\` : '';
      grid += \`<div class="jcc-di\${fc}"><span class="jcc-dlbl">\${lbl}</span><span class="jcc-dval">\${val}
